...
│  Pedagogical feedback: Groq (primary), Anthropic Claude (fallback)  │
└─────────────────────────────────────────────────────────────────────┘
```
...
| Feedback | `services/feedback.py` | Pedagogical feedback via Groq (primary) or Anthropic Claude (fallback); PII scrubbing; mock fallback |
...
5. **Feedback Generation (`services/feedback.py`):**
   ...
   - Call Groq (e.g. Llama/Mistral) or Anthropic Claude.
...
| `GROQ_API_KEY` | Groq LLM provider | **Confidential** |
...
- AI integrations with Wolfram Alpha, Groq, Anthropic, Mathpix.
...
- [ ] Public copy should say “AI providers used include Wolfram Alpha, Groq, Anthropic, Mathpix” without referencing internal fallback chain or mock modes.
